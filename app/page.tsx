...
export default function VolumeCalculator() {
  ...
  const [containerVolumeInput, setContainerVolumeInput] =
    useState<VolumeInputs>({
      volume: "",
    });
  ...
  const [containerInputType, setContainerInputType] = useState<
    "dimensions" | "volume"
  >("dimensions");
  const [voidInputType, setVoidInputType] = useState<"dimensions" | "volume">(
    "dimensions",
  );
  ...
  useEffect(() => {
    ...
    // Load container data
    setContainerInputType(
      (params.get("containerType") as "dimensions" | "volume") || "dimensions",
    );
    ...
    // Load void data
    setVoidInputType(
      (params.get("voidType") as "dimensions" | "volume") || "dimensions",
    );
    setVoidDimensions({
  ...
  const updateUrlParams = () => {
    ...
    if (containerInputType === "dimensions") {
      if (containerDimensions.length)
        params.set("containerLength", containerDimensions.length);
      if (containerDimensions.width)
        params.set("containerWidth", containerDimensions.width);
      if (containerDimensions.height)
        params.set("containerHeight", containerDimensions.height);
    } else {
      if (containerVolumeInput.volume)
        params.set("containerVolume", containerVolumeInput.volume);
    }

    // Void parameters
    params.set("voidType", voidInputType);
    if (voidInputType === "dimensions") {
      if (voidDimensions.length)
        params.set("voidLength", voidDimensions.length);
      if (voidDimensions.width) params.set("voidWidth", voidDimensions.width);
      if (voidDimensions.height)
        params.set("voidHeight", voidDimensions.height);
    } else {
      if (voidVolumeInput.volume)
        params.set("voidVolume", voidVolumeInput.volume);
    }
  ...
  const handleCalculate = () => {
    ...
    if (containerInputType === "dimensions") {
      ...
      const volume = validateNumber(
        containerVolumeInput.volume,
        "Container volume",
      );
      if (volume === null) {
        return;
    ...
    if (voidInputType === "dimensions") {
      if (
        voidDimensions.length ||
        voidDimensions.width ||
        voidDimensions.height
      ) {
        ...
        if (
          ...
          setErrors((prev) => [
            ...prev,
            "If specifying void dimensions, all fields are required",
          ]);
          return;
        }
    ...
    if (voidVolume > containerVolume) {
      setErrors((prev) => [
        ...prev,
        "Void volume cannot be larger than container volume",
      ]);
      return;
    }
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-gray-600">
            Calculate the volume and weight of material needed to fill a space
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Define the outer container that will hold the material
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setContainerInputType(value as "dimensions" | "volume")
                  }
                >
                  <SelectTrigger>
              ...
              {containerInputType === "dimensions" ? (
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setContainerVolumeInput({ volume: e.target.value })
                    }
                  />
                </div>
              )}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Define any internal void that displaces material
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                ...
                <Select
                  value={voidInputType}
                  onValueChange={(value) =>
                    setVoidInputType(value as "dimensions" | "volume")
                  }
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Choose input method" />
              ...
              {voidInputType === "dimensions" ? (
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setVoidVolumeInput({ volume: e.target.value })
                    }
                  />
                </div>
              )}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Specify material density to calculate weight
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="max-w-md">
              ...
              <p className="text-xs text-gray-500 mt-1">
                Common densities: Silicone (1.2), Water (1.0), Aluminum (2.7),
                Steel (7.8)
              </p>
            </div>
          </CardContent>
              ...
              <div className="grid md:grid-cols-4 gap-4">
                <div className="text-center p-4 bg-blue-100 rounded-lg">
                  <div className="text-2xl font-bold text-blue-800">
                    {formatVolume(result.containerVolume)}
                  </div>
                ...
                <div className="text-center p-4 bg-red-100 rounded-lg">
                  <div className="text-2xl font-bold text-red-800">
                    {formatVolume(result.voidVolume)}
                  </div>
                ...
                <div className="text-center p-4 bg-green-100 rounded-lg">
                  <div className="text-2xl font-bold text-green-800">
                    {formatVolume(result.materialVolume)}
                  </div>
                ...
                {materialDensity && (
                  <div className="text-center p-4 bg-purple-100 rounded-lg">
                    <div className="text-2xl font-bold text-purple-800">
                      {formatWeight(result.materialWeight)}
                    </div>
        ...
        <Card className="bg-gray-50">
          ...
          <CardContent className="space-y-2 text-sm text-gray-600">
            <p>
              1. <strong>Container:</strong> Choose input method and enter the
              dimensions or volume of the outer container.
            </p>
            <p>
              2. <strong>Void (Optional):</strong> If there's an object inside
              that displaces material, enter its dimensions or volume.
            </p>
            <p>
              3. <strong>Material Density (Optional):</strong> Enter the
              material's density in g/cm³ to calculate weight.
            </p>
            <p>
              4. <strong>Units:</strong> Dimensions in millimeters (mm), volumes
              in cubic millimeters (mm³), density in g/cm³. All results are
              displayed in cubic centimeters (cm³).
            </p>
            <p>
              5. <strong>URL Persistence:</strong> Your inputs are saved in the
              URL - bookmark or share the link to preserve your calculation.
            </p>
          </CardContent>
        </Card>
      </div>
